...
import FormSection from "@/app/chatql/_components/FormSection";
...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    if (assignedChatModel !== "/chatql") {
      console.log(
        `User assigned to ${assignedChatModel}, redirecting from /chatql`
      );
  ...
  const generateSummary = async (completeHistory: Message[] = chatHistory) => {
    ...
    try {
      ...
      // This will always be positive (for pilot study)
      const responseLower = "positive";
      let finalSummary = "";
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 p-3  -mb-9 max-w-9xl w-full mx-auto h-[100vh] overflow-y-auto bg-indigo-50 border border-indigo-100 shadow-lg">
          ...
          >
            <FormSection
              ...
              onQuizAnswered={() => {
                // Trigger Jane's response after quiz is answered
                const lastMark =
                  chatHistory.filter((m) => m.role === "mark").slice(-1)[0]
                    ?.content ?? "";
                if (lastMark) {
                  generateJaneResponse(lastMark);
                }
              }}
          ...
          <div className="p-8 text-center max-w-4xl">
            ...
            <div className="text-left space-y-4 text-lg text-gray-700 leading-relaxed">
              ...
              <p className="font-semibold text-indigo-700">
                ...
                Jane exchange and answer questions after each set of messages.
              </p>
              <p>
