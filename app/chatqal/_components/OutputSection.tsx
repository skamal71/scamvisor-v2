...
export default function OutputSection({
  ...
  // Reset nextPhaseClicked when phase changes
  useEffect(() => {
  ...
  // Scroll to Mark's first message of the current phase when phase changes
  useEffect(() => {
    if (chatRef.current && currentPhase) {
      // Find Mark's first message in the current phase
      const phaseStartIndex = chatHistory.findIndex(
        (msg) =>
          msg.role === "mark" &&
          (currentPhase === 1
            ? true
      ...
      if (phaseStartIndex !== -1) {
        // Find the first Mark message after the phase start
        const messagesAfterPhaseStart = chatHistory.slice(phaseStartIndex + 1);
        const firstMarkInPhase = messagesAfterPhaseStart.find(
          (msg) =>
            msg.role === "mark" &&
            !msg.content.startsWith("[FEEDBACK]") &&
            !msg.content.startsWith("[SUMMARY]") &&
        ...
        if (firstMarkInPhase) {
          const firstMarkIndex = chatHistory.indexOf(firstMarkInPhase);
          const messageElements = chatRef.current.children;

          if (messageElements[firstMarkIndex]) {
            setTimeout(() => {
              messageElements[firstMarkIndex].scrollIntoView({
                behavior: "smooth",
                block: "start",
              });
            }, 500); // Small delay to ensure the new phase content is rendered
          }
        }
      }
    }
  ...
  if (loading) {
    if (chatHistory.length === 0) {
      typingAgent = "mark"; // Mark types first when chat is empty
    } else {
  ...
  const processMessageContent = (role: string, content: string) => {
    if (role === "mark" || role === "jane") {
      // Remove the first three words from both Mark's and Jane's messages
  ...
  return (
    <div
      ref={chatRef}
      className="bg-white shadow-md rounded-2xl p-5 h-[98vh] overflow-y-auto flex flex-col gap-6 transition-all duration-1000 ease-in-out"
    >
        ...
        for (let i = 0; i < messages.length; i++) {
          ...
          if (messageType === "feedback-summary") {
            ...
            if (nextMessageType === "feedback-summary") {
              ...
              const processedFeedbackContent = toBulletPoints(feedbackContent);
              const processedSummaryContent = toBulletPoints(summaryContent);
                ...
                >
                  <div
                    className={`rounded-2xl max-w-4xl ${
                      feedbackShown
                        ? "p-[4px]"
                        : "px-4 py-3 bg-blue-50 border-2 border-blue-200 text-blue-800"
                    }`}
                    style={{
                      background: feedbackShown
                  ...
                  >
                    <div
                      className={`rounded-2xl max-w-4xl ${
                        feedbackShown ? "bg-blue-50 p-4" : "bg-blue-50 p-4"
                      }`}
                    >
                      <div className="mb-4">
                        <h3 className="font-semibold text-center mb-2 text-md text-blue-800">
                          Feedback
                        </h3>
                        <p
                          className="text-black whitespace-pre-line"
                          dangerouslySetInnerHTML={{
                            __html: processedFeedbackContent,
                          }}
                        ></p>
                      </div>
                      <div>
                        ...
                        <p
                          className="text-black whitespace-pre-line"
                          dangerouslySetInnerHTML={{
                            __html: processedSummaryContent,
                          }}
                        ></p>
                      </div>
                    </div>
              ...
              const processedContent = toBulletPoints(content);
      ...
      {loading && typingAgent === "jane" && (
        <div className="flex justify-end items-start gap-4 animate-pulse">
          <div className="bg-gray-200 p-3 rounded-2xl max-w-lg">
      ...
      {showScrollIndicator && (
        <div className="fixed bottom-4 left-1/2 transform -translate-x-1/2 z-50 animate-bounce">
          <div className="bg-red-500 text-white px-5 py-2 rounded-3xl shadow-lg flex items-center gap-2">
            <svg
